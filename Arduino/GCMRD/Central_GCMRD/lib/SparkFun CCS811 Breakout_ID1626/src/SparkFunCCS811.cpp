...
CCS811Core::status CCS811Core::beginCore(void)
{
	...
	while ( Wire1.available() ) //Clear Wire1 as a precaution
	{
		Wire1.read();
	}
...
CCS811Core::status CCS811Core::readRegister(uint8_t offset, uint8_t* outputPointer)
{
	...
	Wire1.requestFrom(I2CAddress, numBytes);
...
CCS811Core::status CCS811Core::multiReadRegister(uint8_t offset, uint8_t *outputPointer, uint8_t length)
{
	...
	else  //OK, all worked, keep going
	{
		...
		Wire1.requestFrom(I2CAddress, length);
		while ( (Wire1.available()) && (i < length))  // slave may send less than requested
		{
			c = Wire1.read(); // receive a byte as character
			*outputPointer = c;
			outputPointer++;
			i++;
		}
...
CCS811Core::status CCS811Core::multiWriteRegister(uint8_t offset, uint8_t *inputPointer, uint8_t length)
{
	...
	{
		Wire1.write(*inputPointer); // receive a byte as character
		inputPointer++;
		i++;
	}
	if( Wire1.endTransmission() != 0 )
